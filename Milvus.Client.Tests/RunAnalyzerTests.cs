...
public class RunAnalyzerTests : IAsyncLifetime
{
    ...
    public async Task Field_based_mode_rejects_a_field_that_is_not_a_bm25_input()
    {
        ...
        // Milvus 2.6.20+ relaxed the field-based RunAnalyzer check: any analyzer-enabled field (not only a
        // BM25 function input) is accepted. Assert the older rejection on earlier versions and the new
        // successful tokenization on 2.6.20+.
        if (await Client.GetParsedMilvusVersion() >= new Version(2, 6, 20))
        {
            var results = await Client.RunAnalyzerAsync(
                new[] { "hello world" },
