...
# in-process runner. The `dotnet test` VSTest adapter is unreliable on some hosts (it fails
# with "Test process did not return valid JSON (non-object)"), so this script drives the
# built test assembly directly.
#
# Usage:
