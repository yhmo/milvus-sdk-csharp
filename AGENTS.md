...
`examples/` is a single console project (`Milvus.Examples.csproj`, `OutputType=Exe`) referencing
...
- Run one with:
  ```bash
  dotnet run --project examples -- SimpleExample        # or any other example name
  dotnet run --project examples -- RunAnalyzerExample   # full list in Program.cs
  ```
- `Milvus.Examples.csproj` is part of `Milvus.Client.sln` and is compiled by the CI `build-v2` job, so example
  code cannot silently break.
