...
Runnable, per-feature sample programs showing the DTO API in isolation. A single console project
(`Milvus.Examples.csproj`) with one `*Example.cs` class per feature, dispatched by `Program.cs`
from the example name argument. Built with `ProjectReference` to `Milvus.Client.V2.csproj` (source
project), so they run in the repo without any packaging step:
...
- Each example is one `*Example.cs` class with a static `Run(string uri)` entry point, runnable as
  `dotnet run --project examples -- <ExampleName>`, mirroring the java/cpp per-feature sample style.
