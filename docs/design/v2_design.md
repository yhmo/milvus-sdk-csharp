...
- Both packages are published independently; `Milvus.Client.V2` has **no dependency** on
  `Milvus.Client`.
- Both generate gRPC code from the shared `Protos` submodule at the repository root. The generated
  classes are `internal` and use the proto-declared `Milvus.Client.Grpc` namespace; being internal,
  they can never clash between the two assemblies even when both are referenced.
...
```bash
dotnet test --filter "Category=Unit|Category=Integration"   # fast layers, no container
dotnet test --filter "Category=System"                       # e2e, needs the Milvus container
```
...
| `BulkImport.cs` | §4.13 BulkImport | `MilvusBulkImport` static REST client (`CreateImportJobsAsync`, `GetImportJobProgressAsync`, `ListImportJobsAsync`) |
...
| Visibility | Contents |
|---|---|
| **public** | `MilvusClientV2` (facade), `*Req` / `*Resp` DTOs, `Types` (config & data model: `ConnectConfig`, `RetryConfig`, `DataType`, `IndexType`, `SimilarityMetricType`, `FieldSchema`, `CollectionSchema`, `FieldSchema`, `MilvusHealthState`, ...), `MilvusException`, `MilvusErrorCode`, `MilvusTimestampUtils`, the FP16 utility, and `CollectionTsCache` (process-wide singleton kept public for parity with the Java/C++ static caches; see §5.1.2) |
| **internal** | `Milvus.Client.Grpc.*` (generated proto), `InvokeAsync`, the gRPC client, `ToGrpc*()` / `FromGrpc()`, `Utils` helpers (`Verify`, `Constants`, `LoggingExtensions`), caches (`SchemaCache`), `RetryPolicy`, `RestfulClient` |
...
public sealed class Float16VectorFieldData : FieldData<ReadOnlyMemory<ushort>>        // FP16 bit patterns
public sealed class BFloat16VectorFieldData : FieldData<ReadOnlyMemory<ushort>>       // BFloat16 bit patterns
...
| `Float16Vector` | `FieldData<ReadOnlyMemory<ushort>>` / `Float16VectorFieldData` (FP16 bit patterns, little-endian) |
| `BFloat16Vector` | `FieldData<ReadOnlyMemory<ushort>>` / `BFloat16VectorFieldData` (BFloat16 bit patterns) |
...
| `JSON` / dynamic rows | `FieldData<string>` (`CreateJson`), or the nameless dynamic `$meta` column |
...
| name | request | response | description |
|---|---|---|---|
| `InsertAsync` | `InsertReq` | `MutationResp` | Inserts rows; updates `CollectionTsCache` (§5.1.2) |
| `UpsertAsync` | `UpsertReq` | `MutationResp` | Inserts or updates rows; updates `CollectionTsCache` |
| `DeleteAsync` | `DeleteReq` | `MutationResp` | Deletes rows by expression; updates `CollectionTsCache` |
...
gRPC). C# V2 therefore exposes a small static REST client, `MilvusBulkImport`, for this domain
(the gRPC equivalents `Import`/`GetImportState`/`ListImportTasks` exist in the proto but are not
the 2.6 SDK surface). The C# static methods do not take a request DTO; instead they take the
collection name / job id as plain parameters and return the server's raw JSON payload as a
`JsonDocument`.

| Java 2.6 `BulkImportUtils` | C++ 2.6 `BulkImport` | C# `MilvusBulkImport` | Description |
|---|---|---|---|
| `bulkImport(url, BaseImportRequest)` | `CreateImportJobs(url, collectionName, files, ...)` | `CreateImportJobsAsync(url, collectionName, files, ...)` → `JsonDocument` | Creates an import job from data files |
| `getImportProgress(url, BaseDescribeImportRequest)` | `GetImportJobProgress(url, jobId, ...)` | `GetImportJobProgressAsync(url, jobId, ...)` → `JsonDocument` | Gets the progress of an import job |
| `listImportJobs(url, BaseListImportJobsRequest)` | `ListImportJobs(url, collectionName, ...)` | `ListImportJobsAsync(url, collectionName, ...)` → `JsonDocument` | Lists import jobs of a collection |

> The C# REST client talks to the `/v2/vectordb/jobs/import/*` endpoints and returns the raw JSON
> payload so callers can read any server extension fields. Request bodies follow the Java request
> classes (`apiKey` + `options`, etc.) and the C++ method parameters above. (C++ also has 2PC
> `Commit`/`Abort`, but the 2.6 parity scope is the 3 APIs above.)
...
`Types` enums (`Types/DataType.cs`, `Types/IndexType.cs` and `Types/SimilarityMetricType.cs`).
...
| `Geometry` (24) | `GEOMETRY` | Yes |
| `Timestamptz` (26) | `TIMESTAMPTZ` | Yes |
| `BinaryVector` / `FloatVector` / `Float16Vector` | `BINARY_VECTOR`/`FLOAT_VECTOR`/`FLOAT16_VECTOR` | Yes |
| `BFloat16Vector` (103) | `BFLOAT16_VECTOR` | Yes |
| `SparseFloatVector` (104) | `SPARSE_FLOAT_VECTOR` | Yes |
| `Int8Vector` (105) | `INT8_VECTOR` | Yes |
| `Struct` (201) | `STRUCT` | Yes |
...
| `HNSW_SQ` / `HNSW_PQ` / `HNSW_PRQ` | `HNSW_SQ`/`HNSW_PQ`/`HNSW_PRQ` | Yes (as `HnswSq`/`HnswPq`/`HnswPrq`) |
...
| `TRIE` | `TRIE` | Yes |
| `STL_SORT` / `INVERTED` / `BITMAP` | `STL_SORT`/`INVERTED`/`BITMAP` | Yes (`StlSort`/`Inverted`/`Bitmap`) |
| `SPARSE_INVERTED_INDEX` / `SPARSE_WAND` | `SPARSE_INVERTED_INDEX`/`SPARSE_WAND` | Yes (`SparseInvertedIndex`/`SparseWand`) |
| — | `IVF_RABITQ` / `AISAQ` / `MINHASH_LSH` / `NGRAM` / `RTREE` (C++-only) | `IvfRabitq`/`MinHashLsh`/`Ngram`/`Rtree` ship as C# extras; `AISAQ` not in 2.6 parity scope |
...
| `BM25` | `BM25` | Yes |
| `MAX_SIM` / `MAX_SIM_COSINE`/`MAX_SIM_IP`/`MAX_SIM_L2`/`MAX_SIM_JACCARD`/`MAX_SIM_HAMMING` | `MAX_SIM_COSINE`/`MAX_SIM_IP`/`MAX_SIM_L2`/`MAX_SIM_JACCARD`/`MAX_SIM_HAMMING` | Yes (`MaxSim`/`MaxSimCosine`/`MaxSimIp`/`MaxSimL2`/`MaxSimJaccard`/`MaxSimHamming`) |
...
public async Task<MutationResp> InsertAsync(InsertReq request, CancellationToken ct)
{
    MutationResp resp = ...;
...
1. **gRPC transport error code** (`RpcException.StatusCode`) — an identical 8-code blacklist is
   shared by all three SDKs; those are **never** retried:

   | gRPC code | Retry? |
   |---|---|
   | `Cancelled`, `DeadlineExceeded`, `PermissionDenied`, `Unauthenticated`, `InvalidArgument`, `AlreadyExists`, `ResourceExhausted`, `Unimplemented` | **No** (fail immediately; `Cancelled` is caller-initiated) |
   | every other code, notably `Unavailable`, `Unknown`, `Internal`, `NotFound`, ... | **Yes** |
...
public static bool IsRetryable(Exception exception, RetryConfig config)
    => exception switch
    {
        ...
        RpcException { StatusCode: not (StatusCode.Cancelled or StatusCode.DeadlineExceeded
            or StatusCode.PermissionDenied or StatusCode.Unauthenticated or StatusCode.InvalidArgument
            or StatusCode.AlreadyExists or StatusCode.ResourceExhausted or StatusCode.Unimplemented) } => true,
...
> Note: `LegacyRateLimit (49)` is already present in `MilvusErrorCode` and gated on
> `RetryOnRateLimit` for 2.2-server compatibility (see §6).

## 6. Error model

- `MilvusErrorCode`: public enum mirroring server error codes (`Success=0`, `UnexpectedError=1`,
  `RateLimit=8`, `ServiceQuotaExceeded=9`, `LegacyRateLimit=49`, `SchemaMismatch=109`,
  `CollectionNotFound=100`, `SegmentNotFound=600`, `IndexNotFound=700`).
- `MilvusException`: wraps server `common.Status` (`ErrorCode` + `Reason`); also wraps transport
  `RpcException` into a consistent surface (preserving the original gRPC status code on
  `MilvusException.GrpcStatusCode`).
...
│   ├── Response/                # per-response FromGrpc*() conversion
│   ├── Types/                   # enums, FieldSchema, FieldData, cache logic
│   └── *.cs                     # timestamp utils, Float16 utils, Verify
├── Integration/                 # Category=Integration
│   ├── *ApiTests.cs             # one class per domain (Collection/Rbac/Alias/Database/
│   │                            #   Index/Partition/ResourceGroup/Utility/BulkImport/Dql/Dml/Health)
│   ├── MockMilvusServer.cs      # in-process gRPC mock, records each RPC request in Requests{}
│   ├── ClientConstructionTests.cs
│   └── AssemblyInfo.cs          # disables parallelization (shared process-wide caches)
└── System/                      # Category=System (real container)
    ├── CollectionTests.cs  EndToEndTests.cs
...
Integration | facade connectivity, request forwarding (every public `MilvusClientV2` method), error mapping, retry | in-process TestServer gRPC mock | s
...
fast layers locally, `--filter "Category=System"` runs the e2e suite, and combining with the trait or
area name allows targeted regression per feature domain. In CI (`Build.yml`) the V1 and V2 test
projects are separate jobs; the V2 job runs Unit, Integration and System as three distinct steps.
...
Coverage is collected with `coverlet` (the `coverlet.collector` package is declared in
`Directory.Packages.props`) and reported via the cross-platform Cobertura format. CI runs with a
repo-root `coverage.runsettings` that restricts measurement to the SDK assemblies only:

```sh
dotnet test Milvus.Client.V2.Tests --collect "XPlat Code Coverage" --settings coverage.runsettings
```

This produces one `coverage.cobertura.xml` under `<test-project>/TestResults/<run-id>/` (ignored by
`.gitignore`). For a readable local HTML report, convert with `reportgenerator`:
...
Coverage is gated so it reflects the SDK rather than test plumbing:

1. **`coverage.runsettings`** — the collector `Include` restricts measurement to
   `Milvus.Client.dll` / `Milvus.Client.V2.dll`, and `ExcludeByAttribute` /
   `ExcludeByFilter` drop generated code (`GeneratedCodeAttribute`, `Milvus.Client.Grpc.*`,
   `Milvus.Proto.*`), so the percentage measures hand-written SDK source only.
2. **`.codecov.yml`** — CI uploads the Cobertura files with `codecov/codecov-action@v5`
   (anonymous, public repo); `.codecov.yml` ignores `obj/`/`bin/`/generated files and enforces the
   coverage targets (project and patch ≥ 70%), so a coverage regression fails the PR status check.
3. When running only the fast layers (e.g. `--filter "Category=Unit|Category=Integration"`), coverage
   reflects conversion/validation logic and mock-driven flows; the System layer needs the real
   container and is therefore measured in the CI V2 job (which runs all three layers) rather than a
   separate per-PR gate.
...
Runnable, per-feature sample programs showing the DTO API in isolation. Split by SDK generation:
`examples/v2/` holds the V2 console project (`Milvus.Examples.csproj`) with one `*Example.cs` class
per feature, dispatched by `Program.cs` from the example name argument. Built with `ProjectReference`
to `Milvus.Client.V2.csproj` (source project), so they run in the repo without any packaging step;
`examples/v1/` is reserved for legacy V1 samples:

```
examples/
├── v1/                             # reserved for legacy Milvus.Client (V1) examples
│   └── README.md
└── v2/
    ├── Milvus.Examples.csproj      # single console project referencing Milvus.Client.V2
    ├── Program.cs                  # dispatches `dotnet run -- <ExampleName>`
    ├── ExampleHelpers.cs           # shared CreateClient (MILVUS_URI/MILVUS_TOKEN) + ResetCollection
    ├── SimpleExample.cs  GeneralExample.cs            # quickstarts
    ├── AddFieldExample.cs  ArrayFieldExample.cs       # schema evolution / field types
    ├── JsonFieldExample.cs  DynamicFieldExample.cs  NullableFieldExample.cs
    ├── BinaryVectorExample.cs  Int8VectorExample.cs  Float16VectorExample.cs  SparseVectorExample.cs
    ├── PartitionKeyExample.cs  ConsistencyLevelExample.cs  GroupByExample.cs
    ├── UpsertExample.cs  RunAnalyzerExample.cs  AliasExample.cs  RBACExample.cs
    └── README.md                   # prerequisites (Milvus server, connection), run instructions
```

- Each example is one `*Example.cs` class with a static `Run(string uri)` entry point, runnable as
  `dotnet run --project examples/v2 -- <ExampleName>`, mirroring the java/cpp per-feature sample style.
- Examples connect to a configurable endpoint (env vars `MILVUS_URI` / `MILVUS_TOKEN`, defaulting to
  `localhost:19530`), via `ExampleHelpers.CreateClient`.

### 8.2 Tutorial (`tutorial/`)

Planned: step-by-step numbered modules that build on each other, mirroring the C++ `tutorial/N_<topic>`
