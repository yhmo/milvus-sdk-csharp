...
public sealed class MutationResp
{
    private MutationResp(
        ...
        IReadOnlyList<int>? successIndex, IReadOnlyList<int>? errorIndex, long cost)
    {
        LongIds = longIds;
        StringIds = stringIds;
    ...
    internal static MutationResp FromGrpc(Grpc.MutationResult response)
    {
        ...
        return new MutationResp(longIds, stringIds, response.InsertCnt, response.DeleteCnt, response.UpsertCnt,
            response.Timestamp,
            response.SuccIndex.Count > 0 ? response.SuccIndex.Select(x => (int)x).ToList() : null,
            response.ErrIndex.Count > 0 ? response.ErrIndex.Select(x => (int)x).ToList() : null,
    ...
    /// <c>[0..rowCount-1]</c>.
    /// </summary>
    public IReadOnlyList<int>? SuccessIndex { get; }
    ...
    /// </summary>
    public IReadOnlyList<int>? ErrorIndex { get; }
