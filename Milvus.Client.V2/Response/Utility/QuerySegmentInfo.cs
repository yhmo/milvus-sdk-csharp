...
public sealed class QuerySegmentInfo
{
    internal QuerySegmentInfo(
        long segmentId, long collectionId, long partitionId, long memSize, long numRows, string indexName,
        long indexId, SegmentState state, IReadOnlyList<long> nodeIds, SegmentLevel level, bool isSorted,
        long storageVersion)
    {
        SegmentId = segmentId;
    ...
    internal static QuerySegmentInfo FromGrpc(Grpc.QuerySegmentInfo info)
        => new(
            info.SegmentID, info.CollectionID, info.PartitionID, info.MemSize, info.NumRows, info.IndexName,
            info.IndexID, (SegmentState)info.State, info.NodeIds.ToList(), (SegmentLevel)info.Level,
