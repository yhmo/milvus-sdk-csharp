    ...
    internal static CollectionSchema ConvertSchema(Grpc.CollectionSchema grpcSchema)
    {
        ...
        foreach (Grpc.StructArrayFieldSchema grpcStruct in grpcSchema.StructArrayFields)
        {
            ...
            foreach (Grpc.FieldSchema grpcSubField in grpcStruct.Fields)
            {
                ...
                // element_type, and the struct's max capacity in a max_capacity type param.
                structField.MaxCapacity = grpcSubField.TypeParams
                    .Where(p => p.Key == "max_capacity")
                    .Select(p => p.Value)
                    .Where(v => long.TryParse(v, NumberStyles.Integer, CultureInfo.InvariantCulture, out _))
                    .Select(v => (int)long.Parse(v, CultureInfo.InvariantCulture))
                    .FirstOrDefault();

                var subField = new FieldSchema(
                    grpcSubField.Name,
                    grpcSubField.ElementType == Grpc.DataType.None ? DataType.Int8 : (DataType)grpcSubField.ElementType,
