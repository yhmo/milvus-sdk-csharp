...
/// explicit <c>url</c> (the Milvus HTTP endpoint) and <c>apiKey</c> (username:password base64) and return the
/// server's raw JSON payload.
/// </summary>
...
public static class MilvusBulkImport
{
    ...
    public static async Task<JsonDocument> CreateImportJobsAsync(
        ...
        IDictionary<string, object>? options = null, CancellationToken cancellationToken = default)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (json.RootElement.TryGetProperty("code", out JsonElement code) && code.TryGetInt32(out int codeValue)
            && codeValue != 0)
        {
