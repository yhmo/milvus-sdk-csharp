...
public sealed partial class MilvusClientV2
{
    ...
    /// <summary>
    /// Describes a role, its users, its description and its privilege grants.
    /// </summary>
    public async Task<DescribeRoleResp> DescribeRoleAsync(DescribeRoleReq request, CancellationToken cancellationToken = default)
    {
        ...
        Grpc.SelectGrantRequest selectGrantRequest = request.ToGrpcSelectGrantRequest();
        Grpc.SelectGrantResponse grantResponse = await InvokeAsync(
            GrpcClient.SelectGrantAsync, selectGrantRequest, static r => r.Status, cancellationToken).ConfigureAwait(false);

        Grpc.SelectRoleRequest selectRoleRequest = request.ToGrpcSelectRoleRequest();
        Grpc.SelectRoleResponse roleResponse = await InvokeAsync(
            GrpcClient.SelectRoleAsync, selectRoleRequest, static r => r.Status, cancellationToken).ConfigureAwait(false);

        return DescribeRoleResp.FromGrpc(roleResponse, grantResponse);
    }
