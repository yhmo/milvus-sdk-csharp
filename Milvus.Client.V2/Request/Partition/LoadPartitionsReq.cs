using Milvus.Client.V2.Utils;
namespace Milvus.Client.V2.Requests.Partition;

/// <summary>
/// Represents a request to load partitions of a collection into memory.
/// </summary>
public sealed class LoadPartitionsReq
{
    /// <summary>
    /// The name of the collection that contains the partitions to load.
    /// </summary>
    public string CollectionName { get; set; } = "";

    /// <summary>
    /// The names of the partitions to load.
    /// </summary>
    public IReadOnlyList<string> PartitionNames { get; set; } = Array.Empty<string>();

    /// <summary>
    /// The number of replicas to create for the loaded partitions.
    /// </summary>
    public int ReplicaNumber { get; set; } = 1;

    /// <summary>
    /// Whether to wait until the partitions are fully loaded. Defaults to <c>true</c>, matching the C++ and Java
    /// SDKs.
    /// </summary>
    public bool Sync { get; set; } = true;

    /// <summary>
    /// The maximum time in milliseconds to wait for the partitions to load when <see cref="Sync" /> is enabled.
    /// Defaults to 60000. A value of 0 means wait forever.
    /// </summary>
    public long TimeoutMs { get; set; } = 60000;

    /// <summary>
    /// When true, loads with refresh enabled, causing the server to reload the partition metadata.
    /// </summary>
    public bool Refresh { get; set; }

    /// <summary>
    /// When set, only the listed fields are loaded into memory (field partial loading).
    /// </summary>
    public IReadOnlyList<string> LoadFields { get; set; } = Array.Empty<string>();

    /// <summary>
    /// When true, dynamic fields are not loaded even when partial loading is not otherwise requested.
    /// </summary>
    public bool SkipLoadDynamicField { get; set; }

    /// <summary>
    /// The resource groups to load the partition replicas into.
    /// </summary>
    public IReadOnlyList<string> TargetResourceGroups { get; set; } = Array.Empty<string>();

    /// <summary>
    /// The load priority, e.g. <c>"Low"</c>, <c>"Medium"</c> or <c>"High"</c>. When set, it is sent to the server
    /// as the <c>load_priority</c> load parameter (normalized to lower case), matching the Java SDK.
    /// </summary>
    public string? Priority { get; set; }

    internal Grpc.LoadPartitionsRequest ToGrpcLoadPartitionsRequest()
    {
        Verify.NotNullOrWhiteSpace(CollectionName);
        Verify.NotNullOrEmpty(PartitionNames);
        var request = new Grpc.LoadPartitionsRequest
        {
            CollectionName = CollectionName,
            ReplicaNumber = ReplicaNumber,
            Refresh = Refresh,
            SkipLoadDynamicField = SkipLoadDynamicField
        };
        request.PartitionNames.AddRange(PartitionNames);
        request.LoadFields.AddRange(LoadFields);
        request.ResourceGroups.AddRange(TargetResourceGroups);

        if (Priority is not null)
        {
            // The proxy only maps the exact lowercase "low"/"medium"/"high" to the matching priority.
#pragma warning disable CA1308, CA1847 // The server contract requires lowercase load-priority values; the string overload is required for netstandard2.0/net462.
            request.LoadParams[Constants.LoadPriority] = Priority.ToLowerInvariant();
#pragma warning restore CA1308, CA1847
        }

        return request;
    }
}
