...
public sealed class CreateIndexReq
{
    ...
    internal Grpc.CreateIndexRequest ToGrpcCreateIndexRequest(IndexParam index)
    {
        ...
        foreach (KeyValuePair<string, string> parameter in index.ExtraParams)
        {
            // The server requires an integer dimension (floats/numeric strings are rejected), matching the
            // Java SDK's isLegalDimensionValue check.
            if (parameter.Key == Constants.VectorDim
                && !long.TryParse(parameter.Value, NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out _))
            {
                throw new ArgumentException(
                    $"Dimension must be an integer, got: '{parameter.Value}'.", nameof(index));
            }
