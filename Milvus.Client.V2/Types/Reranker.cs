    ...
    where T : FunctionReranker<T>
{
    ...
    /// </summary>
    protected FunctionReranker(string name, FunctionType type)
    {
    ...
    public FunctionScore? ToFunctionScore()
    {
        ...
        var function = new FunctionSchema(_name, Type, Array.Empty<string>(), Array.Empty<string>())
        {
            Params = Params.Count > 0 ? new Dictionary<string, string>(Params) : null
        };
...
public sealed class BoostRerank : FunctionReranker<BoostRerank>
{
    ...
    /// <summary>
    /// Sets the field used to generate a random score.
    /// </summary>
    public BoostRerank WithRandomScoreField(string field) => WithParam("random_score_field", field);

    /// <summary>
    /// Sets the seed for the random score.
    /// </summary>
    public BoostRerank WithRandomScoreSeed(long seed) => WithParam("random_score_seed", seed.ToString(CultureInfo.InvariantCulture));
}
...
public sealed class DecayRerank : FunctionReranker<DecayRerank>
{
    ...
    /// </summary>
    public DecayRerank WithOrigin<T>(T origin) => WithParam("origin", origin?.ToString() ?? "");

    /// <summary>
    /// Sets the no-decay zone around the origin.
    /// </summary>
    public DecayRerank WithOffset<T>(T offset) => WithParam("offset", offset?.ToString() ?? "");

    /// <summary>
    /// Sets the scale: the position at which relevance drops to the decay value.
    /// </summary>
    public DecayRerank WithScale<T>(T scale) => WithParam("scale", scale?.ToString() ?? "");
...
public sealed class ModelRerank : FunctionReranker<ModelRerank>
{
    ...
    /// <summary>
    /// Sets the model service endpoint URL.
    /// </summary>
    public ModelRerank WithEndpoint(string endpoint) => WithParam("url", endpoint);
}
