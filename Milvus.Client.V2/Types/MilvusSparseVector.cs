...
public readonly struct MilvusSparseVector<T> : IEquatable<MilvusSparseVector<T>>
{
    ...
    public MilvusSparseVector(ReadOnlyMemory<int> indices, ReadOnlyMemory<T> values)
    {
        ...
        ReadOnlySpan<int> indexSpan = indices.Span;
        for (int i = 0; i < indexSpan.Length; i++)
        {
            if (indexSpan[i] < 0)
            {
                throw new ArgumentException(
                    $"Sparse vector indices must be non-negative, but {indexSpan[i]} was found at position {i}.");
            }

            if (i > 0 && indexSpan[i] <= indexSpan[i - 1])
            {
                throw new ArgumentException(
                    "Sparse vector indices must be strictly ascending (no duplicate indices).");
            }
        }
    ...
    internal static MilvusSparseVector<T> FromBytes(ReadOnlySpan<byte> bytes)
    {
        ...
        {
            int offset = i * 8;
            uint index = BinaryPrimitives.ReadUInt32LittleEndian(bytes.Slice(offset, 4));
            if (index > int.MaxValue)
            {
                throw new NotSupportedException(
                    $"Sparse vector index {index} at position {i} exceeds the supported {int.MaxValue} maximum.");
            }
