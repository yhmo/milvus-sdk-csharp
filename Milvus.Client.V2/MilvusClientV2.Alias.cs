...
public sealed partial class MilvusClientV2
{
        ...
        CreateAliasReq request, CancellationToken cancellationToken = default)
    {
        ...
        CollectionTsCache.Instance.Copy(_endpoint, CurrentDatabase, request.CollectionName, CurrentDatabase, request.Alias);
        // The alias may later be described/cached by name; drop any stale schema entry for it.
        SchemaCache.Instance.Invalidate(_endpoint, CurrentDatabase, request.Alias);
    }
        ...
        DropAliasReq request, CancellationToken cancellationToken = default)
    {
        ...
        CollectionTsCache.Instance.Invalidate(_endpoint, CurrentDatabase, request.Alias);
    }
