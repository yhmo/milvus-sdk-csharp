...
public sealed partial class MilvusClientV2
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // mirroring the C++ and Java SDKs.
        foreach (IndexDesc index in request.Indexes)
        {
            await CreateIndexAsync(index.ToCreateIndexReq(request.CollectionName), cancellationToken)
                .ConfigureAwait(false);
        }

        await LoadCollectionAsync(
            new LoadCollectionReq { CollectionName = request.CollectionName, Sync = false },
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        InvalidateCollectionCaches(request.CollectionName);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        CollectionCacheKey key = CollectionCacheKey.Create(_endpoint, CurrentDatabase, request.CollectionName);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // cached session timestamp and schema must follow the collection to the target database.
        string targetDatabase = request.TargetDatabaseName ?? CurrentDatabase;
        CollectionTsCache.Instance.Move(_endpoint, CurrentDatabase, request.CollectionName, targetDatabase, request.NewCollectionName);
        SchemaCache.Instance.Move(_endpoint, CurrentDatabase, request.CollectionName, targetDatabase, request.NewCollectionName);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        else if (result.State == LoadState.Loading)
        {
            Grpc.GetLoadingProgressResponse progressResponse = await InvokeAsync(
                    GrpcClient.GetLoadingProgressAsync, new Grpc.GetLoadingProgressRequest
                    {
    ...
    private void InvalidateCollectionCaches(string collectionName)
    {
        SchemaCache.Instance.Invalidate(_endpoint, CurrentDatabase, collectionName);
        CollectionTsCache.Instance.Invalidate(_endpoint, CurrentDatabase, collectionName);

        // Canonical -> aliases: drop every alias-keyed entry whose cached schema resolves back to the
        // operation's collection name.
        foreach (string alias in SchemaCache.Instance.GetAliasKeys(_endpoint, CurrentDatabase, collectionName))
        {
            SchemaCache.Instance.Invalidate(_endpoint, CurrentDatabase, alias);
            CollectionTsCache.Instance.Invalidate(_endpoint, CurrentDatabase, alias);
        }

        // Alias -> canonical: when the operation itself used an alias name, drop the canonical-name entry too.
        if (SchemaCache.Instance.TryGetResolvedCollectionName(_endpoint, CurrentDatabase, collectionName, out string? canonical))
        {
            SchemaCache.Instance.Invalidate(_endpoint, CurrentDatabase, canonical);
            CollectionTsCache.Instance.Invalidate(_endpoint, CurrentDatabase, canonical);
        }
    }
}