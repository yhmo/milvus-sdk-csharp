...
public sealed partial class MilvusClientV2
{
    ...
    // request's Data. Both input shapes cannot be set at once.
        ...
        IReadOnlyList<FieldData> data, CollectionSchema schema, bool isUpsert)
    {
        ...
        // Auto-ID primary key must be all-rows-or-no-rows: insert omits it, upsert requires it.
        ...
        if (primaryKey is { AutoId: true })
        {
            ...
            {
                throw new ArgumentException(
                    $"Insert must not provide the auto-id primary key field '{primaryKey.Name}'.",
                    nameof(data));
            }
        }
    }
