...
internal static class RowDataConverter
{
        ...
        bool isUpsert)
    {
        ...
        foreach (FieldSchema field in schema.Fields)
        {
            ...
            {
                if (field.Nullable || field.DefaultValue is not null)
                {
        ...
        switch (field.DataType)
        {
            case DataType.Bool:
                return field.Nullable
                    ...
                    : FieldData.Create(field.Name, values.Select(ConvertBool).ToList(), field.IsDynamic);
            case DataType.Int8:
                return field.Nullable
                    ? FieldData.Create(field.Name, values.Select(v => v is null ? (sbyte?)null : unchecked((sbyte)Convert.ToInt32(v, CultureInfo.InvariantCulture))).ToList(), field.IsDynamic)
                    : FieldData.Create(field.Name, values.Select(v => unchecked((sbyte)Convert.ToInt32(v, CultureInfo.InvariantCulture))).ToList(), field.IsDynamic);
            case DataType.Int16:
                return field.Nullable
                    ...
                    : FieldData.Create(field.Name, values.Select(v => Convert.ToInt16(v, CultureInfo.InvariantCulture)).ToList(), field.IsDynamic);
            case DataType.Int32:
                return field.Nullable
                    ...
                    : FieldData.Create(field.Name, values.Select(v => Convert.ToInt32(v, CultureInfo.InvariantCulture)).ToList(), field.IsDynamic);
            case DataType.Int64:
                return field.Nullable
                    ...
                    : FieldData.Create(field.Name, values.Select(v => Convert.ToInt64(v, CultureInfo.InvariantCulture)).ToList(), field.IsDynamic);
            case DataType.Float:
                return field.Nullable
                    ...
                    : FieldData.Create(field.Name, values.Select(v => Convert.ToSingle(v, CultureInfo.InvariantCulture)).ToList(), field.IsDynamic);
            case DataType.Double:
                return field.Nullable
                    ...
                    : FieldData.Create(field.Name, values.Select(v => Convert.ToDouble(v, CultureInfo.InvariantCulture)).ToList(), field.IsDynamic);
            case DataType.VarChar:
            case DataType.String:
                return FieldData.CreateVarChar(
                    field.Name,
                    values.Select(v => v is null ? null : Convert.ToString(v, CultureInfo.InvariantCulture)).ToList()!,
                    field.IsDynamic);
            case DataType.Json:
                return FieldData.CreateJson(field.Name, values.Select(EncodeJson).ToList(), field.IsDynamic);
    ...
    private static MilvusSparseVector<float> ToSparseVector(object value)
    {
        ...
        foreach (KeyValuePair<object, object?> pair in value is IDictionary<object, object?> dict
            ? dict
            : value is IDictionary<long, float> longDict ? CastDictionary(longDict)
            : value is IDictionary<int, float> intDict ? CastDictionary(intDict)
            : value is IDictionary<string, object> strDict ? CastStringDictionary(strDict)
    ...
    private static IEnumerable<KeyValuePair<object, object?>> CastDictionary(IDictionary<long, float> dict)
    {
        foreach (KeyValuePair<long, float> pair in dict)
        {
            yield return new KeyValuePair<object, object?>(pair.Key, pair.Value);
        }
    }

    private static IEnumerable<KeyValuePair<object, object?>> CastDictionary(IDictionary<int, float> dict)
    {
    ...
    private static IEnumerable<KeyValuePair<object, object?>> CastStringDictionary(IDictionary<string, object> dict)
    {
    ...
    private static ArrayFieldData<object?> BuildArray(FieldSchema field, List<object?> values)
    {
        ...
        return new ArrayFieldData<object?>(field.Name, rows) { ValidData = values.Any(v => v is null) ? values.Select(v => v is not null).ToArray() : null };
    }
}
