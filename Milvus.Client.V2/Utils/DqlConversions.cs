...
internal static class DqlConversions
{
        ...
        switch (fieldData.FieldCase)
        {
            ...
            case Grpc.FieldData.FieldOneofCase.Scalars:
                ...
                return fieldData.Scalars.DataCase switch
                {
                    ...
                    Grpc.ScalarField.DataOneofCase.GeometryWktData => FieldData.CreateVarChar(
                        fieldData.FieldName, fieldData.Scalars.GeometryWktData.Data),
    ...
    private static List<object?> DecodeStructVectorRow(
        ...
        => elementType switch
        {
            ...
            Grpc.DataType.BinaryVector => ChunkBytes(vector.BinaryVector.ToByteArray(), Math.Max(1, dim / 8)).Select(x => (object?)x).ToList(),
