...
public sealed partial class MilvusClientV2
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        // cursor (pk-based filter) is advanced by consuming batches of rows without returning them.
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        if (metricType == SimilarityMetricType.Invalid)
        {
            ListIndexesResp indexes = await ListIndexesAsync(
                new ListIndexesReq { CollectionName = request.CollectionName },
                cancellationToken).ConfigureAwait(false);
            IndexDescription? index = indexes.Indexes.FirstOrDefault(i => i.FieldName == request.VectorFieldName);
...
#pragma warning restore CA1307
    ...
    private Grpc.SearchRequest BuildSearchRequest(SearchReq request)
    {
        ...
        if (parameters is not null)
        {
            ...
            {
                // Structured function-score reranker (boost/decay/model) maps to schema.FunctionScore.
                grpcRequest.FunctionScore = parameters.FunctionScoreReranker.ToFunctionScore()!.ToGrpcFunctionScore();
            }
        ...
        grpcRequest.Nq = searchByIds
            ? request.Ids!.Count
            : request.Vectors.Count + (request.SparseVectors?.Count ?? 0) + (request.HalfVectors?.Count ?? 0);
...
#pragma warning restore CA1307
    ...
    private static Grpc.SearchRequest BuildHybridSearchSubRequest(SearchReq subRequest)
    {
        ...
        if (parameters is not null)
        {
            ...
            {
                // Structured function-score reranker (boost/decay/model) maps to schema.FunctionScore.
                request.FunctionScore = parameters.FunctionScoreReranker.ToFunctionScore()!.ToGrpcFunctionScore();
            }
            else if (parameters.Rerank is not null)
            {
        ...
        request.Nq = subRequest.Vectors.Count
            + (subRequest.SparseVectors?.Count ?? 0)
            + (subRequest.HalfVectors?.Count ?? 0);
